...
fn executeMoveq(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    const v: i8 = switch (i.src) {
        .Immediate8 => |w| @bitCast(w),
        else => 0,
    };
    m.d[r] = @bitCast(@as(i32, v));
    m.setFlags(m.d[r], .Long);
    m.pc += i.size;
    return 4;
}
fn executeMove(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const v = try getOperandValue(m, i.src, i.data_size);
    try setOperandValue(m, i.dst, v, i.data_size);
    m.setFlags(v, i.data_size);
    m.pc += i.size;
...
fn executeMovea(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    var v = try getOperandValue(m, i.src, i.data_size);
    if (i.data_size == .Word) v = @bitCast(@as(i32, @as(i16, @bitCast(@as(u16, @truncate(v))))));
    m.a[r] = v;
    m.pc += i.size;
...
fn executeAdd(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    if (dir == 0) {
        ...
        const res = d +% s;
        setRegisterValue(&m.d[r], res, i.data_size);
        setArithmeticFlags(m, d, s, res, i.data_size, false);
    } else {
        const r = switch (i.src) {
            .DataReg => |v| v,
            else => 0,
        };
        ...
        const res = d +% s;
        try setOperandValue(m, i.dst, res, i.data_size);
        setArithmeticFlags(m, d, s, res, i.data_size, false);
    }
    m.pc += i.size;
...
fn executeAdda(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    var s = try getOperandValue(m, i.src, i.data_size);
    if (i.data_size == .Word) s = @bitCast(@as(i32, @as(i16, @bitCast(@as(u16, @truncate(s))))));
    m.a[r] = m.a[r] +% s;
    m.pc += i.size;
    return 8 + getEACycles(i.src, i.data_size, true);
}
fn executeAddi(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    setArithmeticFlags(m, d, s, res, i.data_size, false);
    m.pc += i.size;
    return 8 + getEACycles(i.dst, i.data_size, true);
}
fn executeAddq(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const s = switch (i.src) {
    ...
    const res = d +% s;
    try setOperandValue(m, i.dst, res, i.data_size);
    if (i.dst != .AddrReg) setArithmeticFlags(m, d, s, res, i.data_size, false);
    m.pc += i.size;
    return 4;
}
fn executeAddx(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const x: u32 = if (m.getFlag(cpu.M68k.FLAG_X)) 1 else 0;
    const s = try getOperandValue(m, i.src, i.data_size);
    const d = try getOperandValue(m, i.dst, i.data_size);
    const res = d +% s +% x;
    try setOperandValue(m, i.dst, res, i.data_size);
    setArithmeticFlags(m, d, s +% x, res, i.data_size, false);
    m.pc += i.size;
    return 4;
}
fn executeSub(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    if (dir == 0) {
        ...
        const res = d -% s;
        setRegisterValue(&m.d[r], res, i.data_size);
        setArithmeticFlags(m, d, s, res, i.data_size, true);
    } else {
        const r = switch (i.src) {
            .DataReg => |v| v,
            else => 0,
...
fn executeSubq(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    const res = d -% s;
    try setOperandValue(m, i.dst, res, i.data_size);
    if (i.dst != .AddrReg) setArithmeticFlags(m, d, s, res, i.data_size, true);
    m.pc += i.size;
    return 4;
}
fn executeSubx(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    const res = d -% s -% x;
    try setOperandValue(m, i.dst, res, i.data_size);
    setArithmeticFlags(m, d, s +% x, res, i.data_size, true);
    m.pc += i.size;
    return 4;
}
fn executeCmp(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    const s = try getOperandValue(m, i.src, i.data_size);
    const d = getRegisterValue(m.d[r], i.data_size);
    setArithmeticFlags(m, d, s, d -% s, i.data_size, true);
    m.pc += i.size;
    return 4 + getEACycles(i.src, i.data_size, true);
}
fn executeCmpa(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    var s = try getOperandValue(m, i.src, i.data_size);
    if (i.data_size == .Word) s = @bitCast(@as(i32, @as(i16, @bitCast(@as(u16, @truncate(s))))));
    setArithmeticFlags(m, m.a[r], s, m.a[r] -% s, .Long, true);
    m.pc += i.size;
    return 6 + getEACycles(i.src, i.data_size, true);
}
...
fn executeCmpm(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const ay: u3 = @truncate(i.opcode & 7);
    const ax: u3 = @truncate((i.opcode >> 9) & 7);
    const s = try getOperandValue(m, .{ .AddrPostInc = ay }, i.data_size);
    const d = try getOperandValue(m, .{ .AddrPostInc = ax }, i.data_size);
    setArithmeticFlags(m, d, s, d -% s, i.data_size, true);
    m.pc += i.size;
    return 12;
}
...
fn executeMulu(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    const s: u32 = try getOperandValue(m, i.src, .Word);
    const d: u32 = m.d[r] & 0xFFFF;
    const res = s * d;
    m.d[r] = res;
    m.setFlag(cpu.M68k.FLAG_N, (res & 0x80000000) != 0);
    m.setFlag(cpu.M68k.FLAG_Z, res == 0);
...
fn executeMuls(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    const vs = try getOperandValue(m, i.src, .Word);
    const s16: i16 = @bitCast(@as(u16, @truncate(vs)));
    const d16: i16 = @bitCast(@as(u16, @truncate(m.d[r])));
    const res: i32 = @as(i32, s16) * @as(i32, d16);
    m.d[r] = @bitCast(res);
    m.setFlag(cpu.M68k.FLAG_N, res < 0);
    m.setFlag(cpu.M68k.FLAG_Z, res == 0);
    m.setFlag(cpu.M68k.FLAG_V, false);
    m.setFlag(cpu.M68k.FLAG_C, false);
    m.pc += i.size;
...
fn executeDivu(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    const divisor = try getOperandValue(m, i.src, .Word);
    if (divisor == 0) {
        try m.enterException(5, m.pc, 0, null);
        return 38;
    }
    const res = m.d[r] / divisor;
    const rem = m.d[r] % divisor;
    if (res > 0xFFFF) {
        m.setFlag(cpu.M68k.FLAG_V, true);
    } else {
        m.d[r] = (rem << 16) | (res & 0xFFFF);
        m.setFlags(res & 0xFFFF, .Word);
    }
    m.pc += i.size;
    return 76;
}
fn executeDivs(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    const vs = try getOperandValue(m, i.src, .Word);
    const s: i16 = @bitCast(@as(u16, @truncate(vs)));
    if (s == 0) {
        try m.enterException(5, m.pc, 0, null);
        return 38;
    }
    const d: i32 = @bitCast(m.d[r]);
    const res = @divTrunc(d, s);
    const rem = @rem(d, s);
    if (res < -32768 or res > 32767) {
        m.setFlag(cpu.M68k.FLAG_V, true);
    } else {
        const u_rem: u16 = @bitCast(@as(i16, @truncate(rem)));
        const u_res: u16 = @bitCast(@as(i16, @truncate(res)));
        m.d[r] = (@as(u32, u_rem) << 16) | u_res;
        m.setFlags(@as(u32, u_res), .Word);
    }
    m.pc += i.size;
    return 76;
}
fn executeNeg(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const d = try getOperandValue(m, i.dst, i.data_size);
    const res = 0 -% d;
    try setOperandValue(m, i.dst, res, i.data_size);
    setArithmeticFlags(m, 0, d, res, i.data_size, true);
    m.pc += i.size;
    return 4 + getEACycles(i.dst, i.data_size, false);
}
fn executeNegx(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const x: u32 = if (m.getFlag(cpu.M68k.FLAG_X)) 1 else 0;
    const d = try getOperandValue(m, i.dst, i.data_size);
    const res = 0 -% d -% x;
    try setOperandValue(m, i.dst, res, i.data_size);
    setArithmeticFlags(m, 0, d +% x, res, i.data_size, true);
    m.pc += i.size;
    return 4 + getEACycles(i.dst, i.data_size, false);
}
fn executeClr(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    try setOperandValue(m, i.dst, 0, i.data_size);
    m.setFlags(0, i.data_size);
    m.pc += i.size;
    return 4 + getEACycles(i.dst, i.data_size, false);
}
fn executeTst(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const v = try getOperandValue(m, i.dst, i.data_size);
    m.setFlags(v, i.data_size);
    m.pc += i.size;
    return 4 + getEACycles(i.dst, i.data_size, true);
}
fn executeSwap(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const r = switch (i.dst) {
        .DataReg => |v| v,
        else => 0,
    };
    const low = m.d[r] & 0xFFFF;
    const high = m.d[r] >> 16;
    m.d[r] = (low << 16) | high;
    m.setFlags(m.d[r], .Long);
    m.pc += i.size;
...
fn executeExt(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    if (i.is_extb) {
        m.d[r] = @bitCast(@as(i32, @as(i8, @bitCast(@as(u8, @truncate(m.d[r]))))));
    } else if (i.data_size == .Word) {
        const ext_val = @as(i16, @as(i8, @bitCast(@as(u8, @truncate(m.d[r])))));
        m.d[r] = (m.d[r] & 0xFFFF0000) | (@as(u32, @bitCast(@as(i32, ext_val))) & 0xFFFF);
    } else {
        m.d[r] = @bitCast(@as(i32, @as(i16, @bitCast(@as(u16, @truncate(m.d[r]))))));
    }
    m.setFlags(m.d[r], .Long);
    m.pc += i.size;
    return 4;
}
fn executeLea(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const r = switch (i.dst) {
        .AddrReg => |v| v,
        else => 0,
    };
    m.a[r] = try calculateEA(m, i.src);
    m.pc += i.size;
...
fn executeTrap(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const v = switch (i.src) {
        .Immediate8 => |w| w,
        else => 0,
    };
    const vn: u8 = 32 + (v & 0xF);
    try m.enterException(vn, m.pc + 2, 0, null);
    return 34;
}
...
fn executeBra(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const d = switch (i.src) {
        .Immediate8 => |v| @as(i32, @as(i8, @bitCast(v))),
...
fn executeBcc(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    if (evaluateCondition(m, cond)) return try executeBra(m, i);
    m.pc += i.size;
    return branchNotTakenCycles(i.size);
}
...
fn executeJmp(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    m.pc = try calculateEA(m, i.dst);
    return 8;
}
fn executeDbcc(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const cond: u4 = @truncate((i.opcode >> 8) & 0xF);
    const r = @as(u3, @truncate(i.opcode & 7));
    if (!evaluateCondition(m, cond)) {
        const v: i16 = @bitCast(@as(u16, @truncate(m.d[r])));
        const nv = v -% 1;
        setRegisterValue(&m.d[r], @as(u32, @as(u16, @bitCast(nv))), .Word);
        if (nv != -1) return try executeBra(m, i);
    }
    m.pc += 4;
    return 12;
}
fn executeScc(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const cond: u4 = @truncate((i.opcode >> 8) & 0xF);
    const res: u8 = if (evaluateCondition(m, cond)) 0xFF else 0;
    try setOperandValue(m, i.dst, res, .Byte);
    m.pc += i.size;
    return 4 + getEACycles(i.dst, .Byte, false);
}
fn executeShift(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const count = switch (i.src) {
        .Immediate8 => |v| @as(u32, v),
        .DataReg => |r| m.d[r] & 63,
        else => 1,
    };
    var val = try getOperandValue(m, i.dst, i.data_size);
    const mask: u32 = if (i.data_size == .Byte) 0xFF else if (i.data_size == .Word) 0xFFFF else 0xFFFFFFFF;
    const sign: u32 = if (i.data_size == .Byte) 0x80 else if (i.data_size == .Word) 0x8000 else 0x80000000;
    var c = m.getFlag(cpu.M68k.FLAG_C);
    var x = m.getFlag(cpu.M68k.FLAG_X);
    for (0..count) |_| {
        switch (i.mnemonic) {
            .LSR => {
                c = (val & 1) != 0;
...
fn executeBtst(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const b = (try getOperandValue(m, i.src, .Byte)) & 31;
    const v = try getOperandValue(m, i.dst, i.data_size);
    m.setFlag(cpu.M68k.FLAG_Z, (v & (@as(u32, 1) << @truncate(b))) == 0);
    m.pc += i.size;
    return 4 + getEACycles(i.dst, i.data_size, true);
}
fn executeBset(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const b = (try getOperandValue(m, i.src, .Byte)) & 31;
    const v = try getOperandValue(m, i.dst, i.data_size);
    m.setFlag(cpu.M68k.FLAG_Z, (v & (@as(u32, 1) << @truncate(b))) == 0);
    try setOperandValue(m, i.dst, v | (@as(u32, 1) << @truncate(b)), i.data_size);
    m.pc += i.size;
    return 8 + getEACycles(i.dst, i.data_size, false);
}
fn executeBclr(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const b = (try getOperandValue(m, i.src, .Byte)) & 31;
    const v = try getOperandValue(m, i.dst, i.data_size);
    m.setFlag(cpu.M68k.FLAG_Z, (v & (@as(u32, 1) << @truncate(b))) == 0);
    try setOperandValue(m, i.dst, v & ~(@as(u32, 1) << @truncate(b)), i.data_size);
...
fn executeBchg(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const b = (try getOperandValue(m, i.src, .Byte)) & 31;
    const v = try getOperandValue(m, i.dst, i.data_size);
    m.setFlag(cpu.M68k.FLAG_Z, (v & (@as(u32, 1) << @truncate(b))) == 0);
    try setOperandValue(m, i.dst, v ^ (@as(u32, 1) << @truncate(b)), i.data_size);
...
fn executeLink(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const r = switch (i.dst) {
        .AddrReg => |v| v,
        else => 7,
    };
    const d: i16 = @bitCast(switch (i.src) {
        .Immediate16 => |v| v,
        else => 0,
    });
    m.a[7] -= 4;
...
fn executeUnlk(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const r = switch (i.dst) {
        .AddrReg => |v| v,
        else => 7,
    };
...
fn executeExg(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const rx = (i.opcode >> 9) & 7;
    const ry = i.opcode & 7;
    const mode = (i.opcode >> 3) & 0x1F;
    if (mode == 8) {
        const tmp = m.d[rx];
        m.d[rx] = m.d[ry];
        m.d[ry] = tmp;
...
fn executeChk(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const r = switch (i.dst) {
        .DataReg => |v| v,
        else => 0,
    };
    const b = try getOperandValue(m, i.src, .Word);
    const v = m.d[r] & 0xFFFF;
    if (v > b or (v & 0x8000) != 0) {
        try m.enterException(6, m.pc + i.size, 0, null);
        return 44;
    }
    m.pc += i.size;
    return 10;
...
fn executeAbcd(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const s = try getOperandValue(m, i.src, .Byte);
    const d = try getOperandValue(m, i.dst, .Byte);
    const res = addBcd(@truncate(d), @truncate(s), m.getFlag(cpu.M68k.FLAG_X));
    try setOperandValue(m, i.dst, res.result, .Byte);
    m.setFlag(cpu.M68k.FLAG_X, res.carry);
    m.setFlag(cpu.M68k.FLAG_C, res.carry);
    if (res.result != 0) {
        m.setFlag(cpu.M68k.FLAG_Z, false);
    }
    m.pc += i.size;
    return 6;
}
fn executeSbcd(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const s = try getOperandValue(m, i.src, .Byte);
    const d = try getOperandValue(m, i.dst, .Byte);
    const res = subBcd(@truncate(d), @truncate(s), m.getFlag(cpu.M68k.FLAG_X));
    try setOperandValue(m, i.dst, res.result, .Byte);
    m.setFlag(cpu.M68k.FLAG_X, res.carry);
    m.setFlag(cpu.M68k.FLAG_C, res.carry);
    if (res.result != 0) {
...
fn executeNbcd(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const v = try getOperandValue(m, i.dst, .Byte);
    const res = subBcd(0, @truncate(v), m.getFlag(cpu.M68k.FLAG_X));
    try setOperandValue(m, i.dst, res.result, .Byte);
    m.setFlag(cpu.M68k.FLAG_X, res.carry);
    m.setFlag(cpu.M68k.FLAG_C, res.carry);
    if (res.result != 0) {
...
fn executeBftst(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    var bit = offset;
    for (0..width) |_| {
        if ((base & (@as(u32, 1) << @truncate(bit))) != 0) {
            field |= @as(u32, 1) << @truncate(width - 1 - bit + offset);
        }
        bit = (bit + 1) & 31;
    }
    m.setFlag(cpu.M68k.FLAG_Z, field == 0);
    m.setFlag(cpu.M68k.FLAG_N, (field & (@as(u32, 1) << @truncate(width - 1))) != 0);
    m.setFlag(cpu.M68k.FLAG_V, false);
    m.setFlag(cpu.M68k.FLAG_C, false);
    m.pc += 4;
    return bitfieldCycles(i.dst, false);
}
fn executeBfset(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    var mask: u32 = 0;
    for (0..width) |j| {
        mask |= @as(u32, 1) << @truncate((offset + j) & 31);
    }
    const field = val & mask;
    m.setFlag(cpu.M68k.FLAG_Z, field == 0);
    m.setFlag(cpu.M68k.FLAG_N, (val & (@as(u32, 1) << @truncate((offset + width - 1) & 31))) != 0);
    val |= mask;
    try setOperandValue(m, i.dst, val, .Long);
    m.pc += 4;
    return bitfieldCycles(i.dst, true);
}
...
fn executeBfexts(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    const dn = @as(u3, @truncate((ext >> 12) & 7));
    const offset = if ((ext & 0x800) != 0) m.d[@as(u3, @truncate((ext >> 6) & 7))] & 31 else @as(u32, (ext >> 6) & 31);
    const width = if ((ext & 0x20) != 0) ((m.d[@as(u3, @truncate(ext & 7))] - 1) & 31) + 1 else (((ext & 31) - 1) & 31) + 1;
    const val = try getOperandValue(m, i.dst, .Long);
    var field: u32 = 0;
    for (0..width) |j| {
        if ((val & (@as(u32, 1) << @truncate((offset + j) & 31))) != 0) {
            field |= @as(u32, 1) << @truncate(j);
        }
    }
    if ((field & (@as(u32, 1) << @truncate(width - 1))) != 0) {
        field |= ~(@as(u32, 0) >> @truncate(32 - width));
    }
    m.d[dn] = field;
    m.setFlag(cpu.M68k.FLAG_Z, field == 0);
    m.setFlag(cpu.M68k.FLAG_N, (field & 0x80000000) != 0);
    m.pc += 4;
    return if (isMem(i.dst)) 12 else 8;
}
fn executeBfextu(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    m.d[dn] = field;
    m.setFlag(cpu.M68k.FLAG_Z, field == 0);
    m.setFlag(cpu.M68k.FLAG_N, false);
    m.pc += 4;
    return if (isMem(i.dst)) 12 else 8;
}
fn executeBfins(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    val &= ~mask;
    for (0..width) |j| {
        if ((m.d[dn] & (@as(u32, 1) << @truncate(j))) != 0) {
            val |= @as(u32, 1) << @truncate((offset + j) & 31);
        }
    }
    m.setFlag(cpu.M68k.FLAG_Z, (val & mask) == 0);
    m.setFlag(cpu.M68k.FLAG_N, (val & (@as(u32, 1) << @truncate((offset + width - 1) & 31))) != 0);
    try setOperandValue(m, i.dst, val, .Long);
    m.pc += 4;
    return bitfieldCycles(i.dst, true);
...
fn executeBfffo(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    m.d[dn] = ffo;
    m.setFlag(cpu.M68k.FLAG_Z, ffo == offset + width);
    m.setFlag(cpu.M68k.FLAG_N, false);
    m.pc += 4;
    return if (isMem(i.dst)) 14 else 10;
}
fn executeCas(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    ...
    const dc = @as(u3, @truncate(ext & 7));
    const du = @as(u3, @truncate((ext >> 6) & 7));
    const mem = try getOperandValue(m, i.dst, i.data_size);
    const comp = getRegisterValue(m.d[dc], i.data_size);
    if (mem == comp) {
        try setOperandValue(m, i.dst, getRegisterValue(m.d[du], i.data_size), i.data_size);
        m.setFlag(cpu.M68k.FLAG_Z, true);
    } else {
        setRegisterValue(&m.d[dc], mem, i.data_size);
        m.setFlag(cpu.M68k.FLAG_Z, false);
    }
    setArithmeticFlags(m, mem, comp, mem -% comp, i.data_size, true);
    m.pc += 4;
    return 12;
}
fn executeCas2(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const ext1 = switch (i.src) {
        .Immediate16 => |v| v,
        else => 0,
    };
    const ext2 = switch (i.dst) {
        .Immediate16 => |v| v,
        else => 0,
    };
    const r1 = @as(u3, @truncate((ext1 >> 12) & 7));
    const a1 = if ((ext1 & 0x8000) != 0) m.a[r1] else m.d[r1];
    const r2 = @as(u3, @truncate((ext2 >> 12) & 7));
    const a2 = if ((ext2 & 0x8000) != 0) m.a[r2] else m.d[r2];
    const dc1 = @as(u3, @truncate(ext1 & 7));
    ...
    if (m1 == c1 and m2 == c2) {
        ...
        setRegisterValue(&m.d[dc1], m1, i.data_size);
        setRegisterValue(&m.d[dc2], m2, i.data_size);
        m.setFlag(cpu.M68k.FLAG_Z, false);
    }
    m.setFlag(cpu.M68k.FLAG_N, false);
    m.setFlag(cpu.M68k.FLAG_V, false);
...
fn executeRtd(m: *cpu.M68k, i: *const decoder.Instruction) !u32 {
    const disp: i16 = @bitCast(switch (i.src) {
        .Immediate16 => |v| v,
        else => 0,
    });
    m.pc = try m.memory.read32(m.a[7]);
...
fn getRegisterValue(v: u32, s: decoder.DataSize) u32 {
    return switch (s) {
        .Byte => v & 0xFF,
        .Word => v & 0xFFFF,
        .Long => v,
...
fn setRegisterValue(r: *u32, v: u32, s: decoder.DataSize) void {
    switch (s) {
        .Byte => r.* = (r.* & 0xFFFFFF00) | (v & 0xFF),
        .Word => r.* = (r.* & 0xFFFF0000) | (v & 0xFFFF),
        .Long => r.* = v,
    }
}
...
fn getOperandValue(m: *cpu.M68k, op: decoder.Operand, s: decoder.DataSize) !u32 {
    ...
    return switch (op) {
        .DataReg => |r| getRegisterValue(m.d[r], s),
        .AddrReg => |r| m.a[r],
        .Immediate8 => |v| v,
        .Immediate16 => |v| v,
        .Immediate32 => |v| v,
...
fn evaluateCondition(m: *cpu.M68k, c: u4) bool {
    const cv = m.getFlag(cpu.M68k.FLAG_C);
    const vv = m.getFlag(cpu.M68k.FLAG_V);
    const zv = m.getFlag(cpu.M68k.FLAG_Z);
    const nv = m.getFlag(cpu.M68k.FLAG_N);
    return switch (c) {
        0 => true,
        1 => false,
        2 => !cv and !zv,
...
fn addBcd(d: u8, s: u8, x: bool) struct { result: u8, carry: bool } {
    var lo = (d & 0xF) + (s & 0xF) + (if (x) @as(u8, 1) else 0);
    var hi = (d >> 4) + (s >> 4);
    var c = false;
    if (lo > 9) {
        lo += 6;
    ...
    return .{ .result = ((hi & 0xF) << 4) | (lo & 0xF), .carry = c };
}
fn subBcd(d: u8, s: u8, x: bool) struct { result: u8, carry: bool } {
    var lo: i16 = @as(i16, d & 0xF) - @as(i16, s & 0xF) - (if (x) @as(i16, 1) else 0);
    var hi: i16 = @as(i16, d >> 4) - @as(i16, s >> 4);
    if (lo < 0) {
        lo += 10;
        hi -= 1;
    }
...
fn isMem(op: decoder.Operand) bool {
    return switch (op) {
        .DataReg, .AddrReg, .None => false,
        else => true,
    };
}
