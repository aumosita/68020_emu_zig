...
pub const Decoder = struct {
    ...
    fn decodeMoveq(_: *const Decoder, o: u16, _: u32) Instruction {
        ...
        i.opcode = o;
        i.mnemonic = .MOVEQ;
        i.dst = .{ .DataReg = @truncate((o >> 9) & 7) };
        i.src = .{ .Immediate8 = @truncate(o & 0xFF) };
        return i;
    }
    fn decodeBranch(_: *const Decoder, o: u16, pc: u32, rw: *const fn (u32) u16) !Instruction {
        ...
        var i = Instruction.init();
        i.opcode = o;
        const cond: u8 = @truncate((o >> 8) & 0xF);
        const d8: i8 = @bitCast(@as(u8, @truncate(o & 0xFF)));
    ...
    fn decodeMove(self: *const Decoder, o: u16, pc: u32, rw: *const fn (u32) u16) !Instruction {
        ...
        i.mnemonic = .MOVE;
        const h = (o >> 12) & 0xF;
        i.data_size = if (h == 1) .Byte else if (h == 3) .Word else .Long;
    ...
    fn decodeGroup5(self: *const Decoder, o: u16, pc: u32, rw: *const fn (u32) u16) !Instruction {
        ...
        if (low == 0xFA or low == 0xFB or low == 0xFC) {
            ...
            i.mnemonic = .Scc;
            i.dst = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, .Byte);
            i.data_size = .Byte;
        } else {
            i.mnemonic = if (((o >> 8) & 1) == 1) .SUBQ else .ADDQ;
            i.data_size = if (((o >> 6) & 3) == 0) .Byte else if (((o >> 6) & 3) == 1) .Word else .Long;
            var d: u8 = @truncate((o >> 9) & 7);
            if (d == 0) d = 8;
            i.src = .{ .Immediate8 = d };
            i.dst = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, i.data_size);
        }
        i.size = @truncate(cpc - pc);
        return i;
    }
    fn decodeGroup8(self: *const Decoder, o: u16, pc: u32, rw: *const fn (u32) u16) !Instruction {
        ...
        } else if (((o >> 6) & 7) == 3 or ((o >> 6) & 7) == 7) {
            i.mnemonic = if (((o >> 6) & 7) == 3) .DIVU else .DIVS;
            i.dst = .{ .DataReg = @truncate((o >> 9) & 7) };
            i.src = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, .Word);
        } else {
            i.mnemonic = .OR;
            i.data_size = if (((o >> 6) & 3) == 0) .Byte else if (((o >> 6) & 3) == 1) .Word else .Long;
            if (((o >> 8) & 1) == 0) {
    ...
    fn decodeArithmetic(self: *const Decoder, o: u16, pc: u32, rw: *const fn (u32) u16, h: u4) !Instruction {
        ...
        const om = (o >> 6) & 7;
        if ((om & 4) != 0) {
            i.mnemonic = if (h == 0xD) .ADDA else .SUBA;
            i.data_size = if ((om & 1) == 0) .Word else .Long;
            i.src = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, i.data_size);
            i.dst = .{ .AddrReg = @truncate((o >> 9) & 7) };
        } else {
            i.mnemonic = if (h == 0xD) .ADD else .SUB;
            i.data_size = if ((om & 3) == 0) .Byte else if ((om & 3) == 1) .Word else .Long;
            if (((om >> 2) & 1) == 0) {
                i.src = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, i.data_size);
                i.dst = .{ .DataReg = @truncate((o >> 9) & 7) };
            } else {
    ...
    fn decodeGroupB(self: *const Decoder, o: u16, pc: u32, rw: *const fn (u32) u16) !Instruction {
        ...
        if (((o >> 3) & 7) == 1 and ((o >> 8) & 1) == 1 and size_bits <= 2) {
            i.mnemonic = .CMPM;
            i.data_size = if (size_bits == 0) .Byte else if (size_bits == 1) .Word else .Long;
        } else if ((om & 4) != 0) {
            i.mnemonic = .CMPA;
            i.data_size = if ((om & 1) == 0) .Word else .Long;
            i.src = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, i.data_size);
            ...
            i.mnemonic = .CMP;
            i.data_size = if (om == 0) .Byte else if (om == 1) .Word else .Long;
            i.src = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, i.data_size);
            i.dst = .{ .DataReg = @truncate((o >> 9) & 7) };
        }
        i.size = @truncate(cpc - pc);
    ...
    fn decodeGroupC(self: *const Decoder, o: u16, pc: u32, rw: *const fn (u32) u16) !Instruction {
        ...
        } else if (om == 3 or om == 7) {
            i.mnemonic = if (om == 3) .MULU else .MULS;
            i.dst = .{ .DataReg = @truncate((o >> 9) & 7) };
            i.src = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, .Word);
        } else if (om >= 4 and om <= 6) {
            i.mnemonic = .EXG;
            i.data_size = .Long;
        } else {
            i.mnemonic = .AND;
    ...
    fn decodeShiftRotate(self: *const Decoder, o: u16, pc: u32, rw: *const fn (u32) u16) !Instruction {
        ...
        if ((o & 0xF8C0) == 0xE8C0) {
            ...
            i.src = .{ .Immediate16 = rw(cpc) };
            cpc += 2;
            i.dst = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, .Long);
            i.size = @truncate(cpc - pc);
            return i;
        }
        if (((o >> 6) & 3) == 3) {
            const st = (o >> 9) & 3;
            const dir = (o >> 8) & 1;
            i.mnemonic = switch (st) {
                0 => if (dir == 0) .ASR else .ASL,
                1 => if (dir == 0) .LSR else .LSL,
                2 => if (dir == 0) .ROXR else .ROXL,
                3 => if (dir == 0) .ROR else .ROL,
                else => .UNKNOWN,
            };
            i.dst = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, .Word);
            i.data_size = .Word;
        } else {
            const st = (o >> 3) & 3;
            const dir = (o >> 8) & 1;
            ...
            if (((o >> 5) & 1) == 0) {
                ...
                if (d == 0) d = 8;
                i.src = .{ .Immediate8 = d };
            } else i.src = .{ .DataReg = @truncate((o >> 9) & 7) };
            i.dst = .{ .DataReg = @truncate(o & 7) };
        }
        i.size = @truncate(cpc - pc);
        return i;
    ...
    fn decodeGroup0(self: *const Decoder, o: u16, pc: u32, rw: *const fn (u32) u16) !Instruction {
        ...
        if (o == 0x0CFC or o == 0x0EFC) {
            i.mnemonic = .CAS2;
            i.data_size = if (o == 0x0CFC) .Word else .Long;
            i.src = .{ .Immediate16 = rw(cpc) };
            cpc += 2;
            i.dst = .{ .Immediate16 = rw(cpc) };
            cpc += 2;
            i.size = @truncate(cpc - pc);
            return i;
        }
        if ((o & 0x01C0) == 0x00C0 and (o & 0x0800) == 0) {
            const sz = (o >> 9) & 0x3;
            if (sz <= 2) {
                const ext = rw(cpc);
                cpc += 2;
        ...
        if ((o & 0x0138) == 0x0108 and ((o >> 6) & 7) >= 4) {
            ...
            i.mnemonic = .MOVEP;
            i.data_size = if (size_bits == 1) .Word else .Long;
            const rd: u3 = @truncate((o >> 9) & 7);
            const ra: u3 = @truncate(o & 7);
            const d = @as(i16, @bitCast(rw(cpc)));
            cpc += 2;
            const dir_bit = (o >> 7) & 1;
            if (dir_bit == 0) {
                i.src = .{ .AddrDisplace = .{ .reg = ra, .displacement = d } };
                i.dst = .{ .DataReg = rd };
            } else {
                i.src = .{ .DataReg = rd };
                i.dst = .{ .AddrDisplace = .{ .reg = ra, .displacement = d } };
            }
            i.size = @truncate(cpc - pc);
            return i;
        }
        if ((o & 0xFFC0) == 0x0AC0 or (o & 0xFFC0) == 0x0CC0 or (o & 0xFFC0) == 0x0EC0) {
            ...
            if (sz >= 1) {
                i.mnemonic = .CAS;
                i.data_size = if (sz == 1) .Byte else if (sz == 2) .Word else .Long;
                i.src = .{ .Immediate16 = rw(cpc) };
                cpc += 2;
                i.dst = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, i.data_size);
                i.size = @truncate(cpc - pc);
        ...
        if ((o & 0xF1C0) == 0x0100 or (o & 0xF1C0) == 0x0140 or (o & 0xF1C0) == 0x0180 or (o & 0xF1C0) == 0x01C0) {
            ...
            const sb = (o >> 6) & 3;
            i.data_size = if (sb == 0) .Byte else if (sb == 1) .Word else .Long;
            i.src = try self.decodeImmediate(&cpc, rw, i.data_size);
            i.dst = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, i.data_size);
        }
        i.size = @truncate(cpc - pc);
        return i;
    ...
    fn decodeGroup4(self: *const Decoder, o: u16, pc: u32, rw: *const fn (u32) u16) !Instruction {
        ...
        var i = Instruction.init();
        i.opcode = o;
        if (o == 0x4E71) i.mnemonic = .NOP else if (o == 0x4E75) i.mnemonic = .RTS else if (o == 0x4E73) i.mnemonic = .RTE else if (o == 0x4E77) i.mnemonic = .RTR else if (o == 0x4E70) i.mnemonic = .RESET else if (o == 0x4E76) i.mnemonic = .TRAPV else if (o == 0x4E72) {
            i.mnemonic = .STOP;
            i.src = .{ .Immediate16 = rw(cpc) };
            cpc += 2;
        } else if (o == 0x4E74) {
            ...
            i.src = .{ .Immediate16 = rw(cpc) };
            cpc += 2;
        } else if (o == 0x4E7A or o == 0x4E7B) {
            i.mnemonic = .MOVEC;
            const ext = rw(cpc);
            cpc += 2;
            i.control_reg = ext & 0xFFF;
            i.is_to_control = (o == 0x4E7B);
            const rn = @as(u3, @truncate((ext >> 12) & 7));
            if (((ext >> 15) & 1) != 0) i.src = .{ .AddrReg = rn } else i.src = .{ .DataReg = rn };
        ...
        } else if ((o & 0xFFF8) == 0x4848) {
            i.mnemonic = .BKPT;
            i.src = .{ .Immediate8 = @truncate(o & 7) };
        } else if ((o & 0xFFF0) == 0x4E40) {
            i.mnemonic = .TRAP;
            i.src = .{ .Immediate8 = @truncate(o & 0xF) };
        } else if ((o & 0xFFF8) == 0x4E50) {
            i.mnemonic = .LINK;
            i.dst = .{ .AddrReg = @truncate(o & 7) };
            i.src = .{ .Immediate16 = rw(cpc) };
            cpc += 2;
        } else if ((o & 0xFFF8) == 0x4E58) {
            i.mnemonic = .UNLK;
            i.dst = .{ .AddrReg = @truncate(o & 7) };
        } else if ((o & 0xFF00) == 0x4E00) {
            if ((o & 0xFFC0) == 0x4E80) i.mnemonic = .JSR else if ((o & 0xFFC0) == 0x4EC0) i.mnemonic = .JMP;
            i.dst = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, .Long);
        ...
        } else if ((o & 0xFFC0) == 0x4800) {
            i.mnemonic = .NBCD;
            i.data_size = .Byte;
            i.dst = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, .Byte);
        } else if ((o & 0xFF00) == 0x4000 or (o & 0xFF00) == 0x4200 or (o & 0xFF00) == 0x4400 or (o & 0xFF00) == 0x4600) {
            const so = (o >> 9) & 7;
            i.mnemonic = switch (so) {
                0 => .NEGX,
                1 => .CLR,
        ...
        } else if ((o & 0xFB80) == 0x4880) {
            i.mnemonic = .MOVEM;
            i.data_size = if ((o & 0x40) != 0) .Long else .Word;
            i.src = .{ .Immediate16 = rw(cpc) };
            cpc += 2;
            i.dst = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, i.data_size);
        } else if ((o & 0xFFC0) == 0x4840) {
            if (((o >> 3) & 7) == 0) {
                i.mnemonic = .SWAP;
                i.dst = .{ .DataReg = @truncate(o & 7) };
            } else {
        ...
        } else if ((o & 0xFFC0) == 0x4880 or (o & 0xFFC0) == 0x48C0 or (o & 0xFFC0) == 0x49C0) {
            i.mnemonic = .EXT;
            i.dst = .{ .DataReg = @truncate(o & 7) };
            const om = (o >> 6) & 7;
            if (om == 7) {
        ...
        } else if ((o & 0xF1C0) == 0x41C0) {
            i.mnemonic = .LEA;
            i.dst = .{ .AddrReg = @truncate((o >> 9) & 7) };
            i.src = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, .Long);
        } else if ((o & 0xF1C0) == 0x4180) {
            i.mnemonic = .CHK;
            i.data_size = .Word;
            i.dst = .{ .DataReg = @truncate((o >> 9) & 7) };
            i.src = try self.decodeEA(@truncate((o >> 3) & 7), @truncate(o & 7), &cpc, rw, .Word);
    ...
    fn decodeEA(self: *const Decoder, mode: u3, reg: u3, pc: *u32, rw: *const fn (u32) u16, size: DataSize) !Operand {
        switch (mode) {
            ...
            5 => {
                const d = @as(i16, @bitCast(rw(pc.*)));
                pc.* += 2;
                return .{ .AddrDisplace = .{ .reg = reg, .displacement = d } };
            },
            6 => return try self.decodeFullExtension(reg, false, pc, rw),
            7 => switch (reg) {
                0 => {
                    const a = @as(i16, @bitCast(rw(pc.*)));
                    pc.* += 2;
                    return .{ .Address = @bitCast(@as(i32, a)) };
                },
                1 => {
                    const h = rw(pc.*);
                    const l = rw(pc.* + 2);
                    pc.* += 4;
                    return .{ .Address = (@as(u32, h) << 16) | l };
                },
                2 => {
                    const d = @as(i16, @bitCast(rw(pc.*)));
                    pc.* += 2;
                    return .{ .ComplexEA = .{ .base_reg = null, .is_pc_relative = true, .index_reg = null, .base_disp = d, .outer_disp = 0, .is_mem_indirect = false, .is_post_indexed = false } };
                },
                3 => return try self.decodeFullExtension(0, true, pc, rw),
                4 => return try self.decodeImmediate(pc, rw, size),
    ...
    fn decodeFullExtension(_: *const Decoder, reg: u3, is_pc: bool, pc: *u32, rw: *const fn (u32) u16) !Operand {
        ...
        if ((ext & 0x0100) == 0) {
            ...
            const idx = @as(u3, @truncate((ext >> 12) & 7));
            const is_l = (ext & 0x0800) != 0;
            const sc = @as(u4, @truncate(@as(u32, 1) << @as(u5, @truncate((ext >> 9) & 3))));
            const d = @as(i8, @bitCast(@as(u8, @truncate(ext & 0xFF))));
            ...
            const bs = (ext & 0x0080) != 0;
            const is = (ext & 0x0040) != 0;
            const bds = (ext >> 4) & 3;
            const iis = ext & 7;
            var bd: i32 = 0;
            if (bds == 2) {
                bd = @as(i16, @bitCast(rw(pc.*)));
                pc.* += 2;
            } else if (bds == 3) {
                bd = @bitCast((@as(u32, rw(pc.*)) << 16) | rw(pc.* + 2));
                pc.* += 4;
            }
            var ir: ?IndexReg = null;
            if (!is) {
                const isa = (ext & 0x8000) != 0;
                const idx = @as(u3, @truncate((ext >> 12) & 7));
                const isl = (ext & 0x0800) != 0;
                const sc = @as(u4, @truncate(@as(u32, 1) << @as(u5, @truncate((ext >> 9) & 3))));
                ir = IndexReg{ .reg = idx, .is_addr = isa, .is_long = isl, .scale = sc };
            }
            var od: i32 = 0;
            const ods = iis & 3;
            if (ods == 2) {
                od = @as(i16, @bitCast(rw(pc.*)));
                pc.* += 2;
            } else if (ods == 3) {
                od = @bitCast((@as(u32, rw(pc.*)) << 16) | rw(pc.* + 2));
                pc.* += 4;
            }
    ...
    fn decodeImmediate(_: *const Decoder, pc: *u32, rw: *const fn (u32) u16, size: DataSize) !Operand {
        switch (size) {
            .Byte => {
                const v = rw(pc.*);
                pc.* += 2;
                return .{ .Immediate8 = @truncate(v) };
            },
            .Word => {
                const v = rw(pc.*);
                pc.* += 2;
                return .{ .Immediate16 = v };
            },
            .Long => {
                const h = rw(pc.*);
                const l = rw(pc.* + 2);
                pc.* += 4;
                return .{ .Immediate32 = (@as(u32, h) << 16) | l };
            },
        }
    }
};
