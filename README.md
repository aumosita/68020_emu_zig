...
- **분석 및 디버깅 도구**: 명령어 실행 이력과 MMIO 상호작용을 기록하는 **Tracer** 시스템을 구현하고, SCC(시리얼) 출력을 호스트 콘솔로 리다이렉트하여 실시간 로그 확인이 가능합니다.
- **SCSI 고도화**: 가상 SCSI 장치 인터페이스와 하드디스크 에뮬레이션을 통해 부팅 초기 과정의 `INQUIRY` 및 `TEST UNIT READY` 응답을 지원합니다.

## 범위와 제한

- 본 프로젝트는 기능 중심 CPU 코어(ISS)에 가깝습니다.
- **68881/68882 FPU는 구현하지 않으며, 향후에도 구현 계획이 없습니다.** F-line은 기본적으로 코프로세서 미사용 예외 경로이며, PMMU-ready 최소 호환 모드에서 coprocessor-id 0만 제한적으로 흡수합니다.
